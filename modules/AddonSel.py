...
import os
import mimetypes
from PyQt6.QtWidgets import QApplication, QDialog, QVBoxLayout, QPushButton, QFileDialog, QLabel, QMessageBox

class Addon:
    def __init__(self,data):
        if isinstance(data,str):
            try:
                id,name,ftype,path = split(";")
    ...
    def get_type(self):
        ...
        elif self.type.startswith("image"): return "image"
        elif self.type.startswith("sound"): return "sound"
        else: return None
...
class AddonSel(QDialog):
    ...
    def process_files(self, file_list):
        ...
        for i,file_path in enumerate(file_list,1):
            ...
            if mime and (mime.startswith('video') or mime.startswith("image")):
                data_rec = {"Id": i, "Name":file_path.split("/")[-1], "Type": mime, "Path": file_path }
    ...
    def process_single_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Media File", "", "Media Files (*.mp4 *.jpg *.png *.jpeg *.mov)")
        if file_path:
